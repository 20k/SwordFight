...
void load_map(objects_container* obj, int width, int height)
{
    ...
    for(int y=0; y<height; y++)
    {
        for(int x=0; x<width; x++)
        {
            ...
            float yoffset = sin(((float)y/height) * M_PI);

            float height = sin(((float)x/width) * M_PI) + yoffset;
...
int main(int argc, char *argv[])
{
    ...
    window.append_opencl_extra_command_line("-DSSAO_RAD=2");
    window.append_opencl_extra_command_line("-DAMBIENT=0.6f");
    ...
    l.set_brightness(1);
    l.radius = 100000;
    l.set_pos((cl_float4){4000, 3000, 3000, 0});
    l.set_diffuse(0.f);
    //l.set_pos((cl_float4){-200, 500, -100, 0});

    light::add_light(&l);

    l.set_pos({1000, 4000, 5000});
    l.set_col({0.3f, 1.0f, 0.3f});
